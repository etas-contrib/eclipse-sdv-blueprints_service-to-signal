...
async fn request_apply(
    request: Option<ActivateHornRequest>,
    tx_kuksa: tokio::sync::mpsc::Sender<bool>,
) -> Option<Option<ActivateHornRequest>> {
    match request {
        Some(request_inner) => {
...
pub async fn horn_request_apply(
    req: ActivateHornRequest,
    tx_kuksa: tokio::sync::mpsc::Sender<bool>,
) {
...
pub async fn horn_sequence_apply(
    sequences: Vec<HornSequence>,
    tx_kuksa: tokio::sync::mpsc::Sender<bool>,
) {
    for sequence in sequences {
        for cycle in sequence.horn_cycles {
            ...
            let _ = tx_kuksa.send(true).await;
            let _ =
                tokio::time::sleep(std::time::Duration::from_millis(cycle.on_time as u64)).await;
            let _ = tx_kuksa.send(false).await;
            let _ =
                tokio::time::sleep(std::time::Duration::from_millis(cycle.off_time as u64)).await;
        }
    }
}
